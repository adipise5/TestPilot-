...
import com.testpilot.testing.report.*;
...
public class DraftExecutionService {
    ...
    private final TestReportBuilder reports;
    ...
    public DraftExecutionService(DraftExecutionRepository store, ProjectService projects, TestPlanningService planning,
                                 SecureContainerExecutor executor, ObjectMapper mapper, TestReportBuilder reports, TestDraftRepository drafts) {
        this.store = store; this.projects = projects; this.planning = planning; this.executor = executor; this.mapper = mapper; this.reports = reports; this.drafts = drafts;
    }
    ...
    public Response execute(Long projectId, Long draftId, UserPrincipal user) {
        ...
        var context = reports.freeze(drafts.findById(draftId).orElseThrow(), input);
        execution.freezeContext(encode(context));
        ...
        execution.finish(encode(result), encode(reports.build(projectId, draftId, context, result, execution.getStartedAt())));
    ...
    public void recoverLostExecutions() {
        for (var execution : store.findByStatusAndStartedAtBefore("RUNNING", LocalDateTime.now().minusSeconds(100))) {
            ...
            var result = SandboxResult.failure("INFRASTRUCTURE_FAILURE", "Worker/server was interrupted; scoped container cleanup requested. Retry explicitly.");
            String report = null;
            if (execution.getReportContextJson() != null) {
                try {
                    var context = mapper.readValue(execution.getReportContextJson(), ReportContext.class);
                    report = encode(reports.build(execution.getProjectId(), execution.getDraftId(), context, result, execution.getStartedAt()));
                } catch (java.io.IOException ex) { /* Old/corrupt context cannot supply trustworthy source evidence. */ }
            }
            execution.finish(encode(result), report);
            try { store.saveAndFlush(execution); }
            catch (ObjectOptimisticLockingFailureException ignored) { }
        }
    }
    public TestReport report(Long projectId, Long draftId, UserPrincipal user) {
        projects.findProjectAndVerifyReadAccess(projectId, user);
        var execution = store.findByDraftIdAndProjectId(draftId, projectId)
                .orElseThrow(() -> new ResourceNotFoundException("No execution for this draft"));
        if (!"COMPLETED".equals(execution.getStatus()) || execution.getReportJson() == null)
            throw new ResourceNotFoundException("No saved report for this attempt. Wait for completion or explicitly rerun a pre-Phase-4 execution.");
        try { return mapper.readValue(execution.getReportJson(), TestReport.class); }
        catch (java.io.IOException ex) { throw new IllegalStateException("Invalid stored test report", ex); }
    }
